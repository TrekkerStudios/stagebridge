...
class AudioFileMapper:
    ...
    def _scan_audio_files(self):
        ...
        if not self.bounced_files_folder.exists():
            print(
                f"Warning: Bounced Files folder not found: {self.bounced_files_folder}"
            )
            return
        ...
        print(
            f"Found {len(wav_files)} audio files in {self.bounced_files_folder}"
        )
        ...
        for track_name, patterns in self.TRACK_MAPPINGS.items():
            ...
            if found_file:
                ...
                print(
                    f"Mapped '{track_name}' -> '{found_file.name}' (duration: {duration:.3f}s)"
                )
            else:
    ...
    def _get_audio_duration(self, audio_file: Path) -> float:
        ...
        try:
            ...
            print(
                f"Warning: Could not get duration for {audio_file.name}: {e}"
            )
            return 0.0

    def get_max_duration(self) -> float:
        """Get the maximum duration found among all audio files."""
        if not self.audio_durations:
            return 0.0
        return max(self.audio_durations.values())
